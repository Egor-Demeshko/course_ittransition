<script>
    import BreadCrumbs from "$lib/components/BreadCrumbs/BreadCrumbs.svelte";
    import AddCollection from "$components/Controlls/ButtonWithIcon.svelte";
    import Collections from "$lib/components/collection/Collections.svelte";
    import Paginator from "$components/Paginator.svelte";
    import { ADD_COLLECTION } from "$data/texts";
    import { addCollection } from "$collections/addCollection";
</script>

<div class="wrapper">
    <BreadCrumbs />
    <div class="add_button__position">
        <AddCollection clickHandler={addCollection}>
            <svg
                slot="icon"
                viewBox="0 0 16 16"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
            >
                <path
                    d="M15 7H9V1C9 0.734784 8.89464 0.48043 8.70711 0.292893C8.51957 0.105357 8.26522 0 8 0C7.73478 0 7.48043 0.105357 7.29289 0.292893C7.10536 0.48043 7 0.734784 7 1V7H1C0.734784 7 0.48043 7.10536 0.292893 7.29289C0.105357 7.48043 0 7.73478 0 8C0 8.26522 0.105357 8.51957 0.292893 8.70711C0.48043 8.89464 0.734784 9 1 9H7V15C7 15.2652 7.10536 15.5196 7.29289 15.7071C7.48043 15.8946 7.73478 16 8 16C8.26522 16 8.51957 15.8946 8.70711 15.7071C8.89464 15.5196 9 15.2652 9 15V9H15C15.2652 9 15.5196 8.89464 15.7071 8.70711C15.8946 8.51957 16 8.26522 16 8C16 7.73478 15.8946 7.48043 15.7071 7.29289C15.5196 7.10536 15.2652 7 15 7Z"
                    fill="currentColor"
                />
            </svg>
            <span slot="text">{ADD_COLLECTION}</span>
        </AddCollection>
    </div>

    <Collections />
</div>
<Paginator />

<style>
    .wrapper {
        max-width: 1280px;
        margin: 0 auto;
        position: relative;
    }

    .add_button__position {
        position: absolute;
        right: 0;
        top: 1rem;
    }

    svg {
        aspect-ratio: 1 / 1;
        width: 16px;
    }

    @media screen and (max-width: 580px) {
        .add_button__position {
            position: fixed;
            left: 1rem;
            right: 1rem;
            bottom: 1rem;
        }
    }
</style>
